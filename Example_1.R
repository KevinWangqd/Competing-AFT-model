...
eta_mat_Ex1$index
data$index <- eta_mat_Ex1$index
data$index <- as.factor(data$index)
# Create a 3D scatter plot
plot_ly(data = data, x = ~X1, y = ~X2, z = ~X3, color = ~index, type = "scatter3d", mode = "markers") %>%
  layout(title = "3D Scatter Plot with Group Colors", scene = list(xaxis = list(title = "X-axis"),
